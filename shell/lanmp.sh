...
function mysql_ins {
	...
	cd mysql-$MYS_VER/
	make_clean
	./configure --prefix=$IN_DIR/mysql-$MYS_VER --sysconfdir=$IN_DIR/etc --enable-assembler --enable-thread-safe-client --with-extra-charsets=complex --with-ssl --with-embedded-server $LIBNCU 
...
function apache_ins {
	...
	cd httpd-$APA_VER
	make_clean
	./configure --prefix=$IN_DIR/httpd-$APA_VER --enable-rewrite --enable-deflate --disable-userdir --enable-so --enable-expires --enable-headers
...
function nginx_ins {
	...
	cd nginx-$NGI_VER
	make_clean
	./configure --user=www --group=www --prefix=$IN_DIR/nginx-$NGI_VER --with-http_stub_status_module --with-http_ssl_module
...
function php_ins {
	...
	NV=""
	[ $SERVER == "nginx" ] && NV="--enable-fastcgi --enable-fpm --with-fpm-conf=$IN_DIR/etc/php-fpm.conf" && gzip -cd php-$PHP_VER-fpm-0.5.14.diff.gz | patch -fd php-$PHP_VER -p1 > $IN_LOG 2>&1
	...
	mkdir -p $IN_DIR/$PHP_DIR/lib/php/extensions/no-debug-zts-20060613
        ln -s $IN_DIR/$PHP_DIR/lib/php/extensions/no-debug-zts-20060613 $IN_DIR/$PHP_DIR/lib/php/extensions/no-debug-non-zts-20060613
...
function start_srv {
	...
	service pureftpd start
	###
	/sbin/iptables -I INPUT -p tcp -m state -m tcp --dport 20000:20500 --state NEW -j ACCEPT
