...
if [ $OS_RL == 2 ];then
	apt-get install -y gcc g++ make autoconf libltdl-dev libgd2-xpm-dev libfreetype6 libfreetype6-dev libxml2-dev libjpeg-dev libpng12-dev libssl-dev libcurl4-openssl-dev patch libmcrypt-dev libmhash-dev libncurses5-dev bzip2 libcap-dev ntpdate chkconfig unzip
else
...
if [ $X86 == 1 ];then
	...
	yum install -y gcc gcc-c++ make sudo autoconf libtool-ltdl-devel gd-devel freetype-devel libxml2-devel libjpeg-devel libpng-devel openssl-devel curl-devel patch libmcrypt-devel libmhash-devel ncurses-devel bzip2 libcap-devel ntp sysklogd unzip
fi
...
function php_ins {
	...
	./configure \
		...
		--with-curl --with-zlib --enable-ftp --with-gd --enable-gd-native-ttf --enable-mbstring --enable-zip --without-iconv
